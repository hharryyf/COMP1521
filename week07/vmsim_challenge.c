...
int physicalAddress(uint vAddr, char action)
{
	int pagenum, offset, i, pAddr = -1, tmppage, min = clock;
	int find = 0;
	...
	if (PageTable[pagenum].status != NotLoaded) {
		...
		if (action == 'W' && PageTable[pagenum].status != Modified) {
		...
		if (find == 0) {  // case for a page replacement
			for (i = 0 ; i < nPages; i++) {
				if (PageTable[i].lastAccessed != -1 && PageTable[i].lastAccessed < min && PageTable[i].status == 	Loaded) {
			...
			if (find == 0) {
				min = clock;
				for (i = 0 ; i < nPages; i++) {
				if (PageTable[i].lastAccessed != -1 && PageTable[i].lastAccessed < min) {
					min = PageTable[i].lastAccessed;
					tmppage = i;
				}
				}
			}
			if (PageTable[tmppage].status == Modified) {
				nSaves++;
			}
			PageTable[tmppage].lastAccessed = -1;
			PageTable[tmppage].frameNo = -1;
			PageTable[tmppage].status = NotLoaded;
			
			for (i = 0 ; i < nFrames; i++) {
				if (MemFrames[i] == tmppage) {
					MemFrames[i] = pagenum;
					PageTable[pagenum].frameNo = i;
					if (action == 'R') {
						PageTable[pagenum].status = Loaded;
