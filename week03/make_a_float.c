...
Union32 getBits(char *sign, char *exp, char *frac)
{
  ...
  // int sign1, exp1, frac1;
   ...
   int i;
   for (i = 7; i >= 0; i--) {
   		new.bits.exp = new.bits.exp + ((exp[i] - '0') << (7 - i));
   }
   // new.bits.exp -= 127;
   // convert char *exp into an 8-bit value in new.bits
   for (i = 22; i >= 0; i--) {
   		new.bits.frac = new.bits.frac + ((frac[i] - '0') << (22 - i));
   }
   // convert char *frac into a 23-bit value in new.bits
  // exp1 = (int) new.bits.exp;
 //  sign1 = (int) new.bits.sign;
 //  frac1 = (int) new.bits.frac;
   
 //  new.xval = (sign1 << 31) + (exp1 << 23) + frac1;
  // printf("%u\n", new.xval);
  // printf("%0.10f\n", ans);
   //new.fval = (1 << new.bits.exp) * (1.0 + ((new.bits.frac) * 1.0) / (1 << 23));
   return new;
}
