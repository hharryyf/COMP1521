...
void send_date(int fd)
{
   ...
   t = time(&t);
   char *str = ctime(&t);
   char *content = calloc(strlen(str) + 14, sizeof(char));
   sprintf(content, "<h1>%s</h1>", str);
   // free(str); 
   send_response(fd, "HTTP/1.1 200 OK",
                     "text/html",
                     content);
...
void send_hello(int fd, char *req)
{
   if (strlen(req) == 6) {
      ...
      char *str = calloc(strlen(req) + 4, sizeof(char));
      char *content;
      int i, n = 1;
      str[0] = 'H';
      for (i = 2; i < strlen(req); i++) {
      	  if (i != 6) {
      	     str[n++] = req[i];
      	  } else {
      	     str[n++] = ',';
      ...
      str[n] = '!';
   	  content = calloc(strlen(str) + 9, sizeof(char));
   	  sprintf(content, "<h1>%s</h1>", str);
   	  send_response(fd, "HTTP/1.1 200 OK",
                     "text/html",
                     content);
...
void handle_http_request(int fd)
{
   ...
   if (strcmp(req_type, "GET") != 0 && strcmp(req_type, "POST") != 0) {
   	   send_500(fd);
   	   return;
   }
   
   if (strncmp(req_path, "/hello", 6) == 0) {//sscanf(req_path, "/hello?%s", string);
   	   send_hello(fd, req_path);
   	   return;
   }
   
   if (strcmp(req_path, "/date") == 0) {
   	  send_date(fd);
   	  return;
   }
